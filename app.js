...
app.use((req, res, next) => {
  res.header('Access-Control-Allow-Origin', 'https://huy-sneaker-store.netlify.app');
  res.header(
    'Access-Control-Header',
    'Origin, X-Requested-With, Content-Type, Accept, Authorization'
  );
...
io.on('connection', (socket) => {
  ...
  socket.on('countUserOnline', async (id) => {
    try {
      ...
      const resultUpdate = await View.findByIdAndUpdate(
        process.env.ID_VIEW,
        { aaa: resultView.aaa + 1 },
        { new: true }
      );
      if (check) {
    ...
    try {
      ...
      if (user) {
        ...
        if (send === 'repLyComment') {
          ...
          const oneStarsResult = await Comment.find({
            id_product: id_product,
            start: 1,
          });
          const twoStarsResult = await Comment.find({
            id_product: id_product,
            start: 2,
          });
          const threeStarsResult = await Comment.find({
            id_product: id_product,
            start: 3,
          });
          const fourStarsResult = await Comment.find({
            id_product: id_product,
            start: 4,
          });
          const fiveStarsResult = await Comment.find({
            id_product: id_product,
            start: 5,
          });
          const sumStarRating =
  ...
  socket.on('disconnect', async () => {
    ...
    const resultView = await View.findById(process.env.ID_VIEW);
    io.sockets.emit('severCountUserOnline', {
      accountOnline: countUserOnline.length,
      view: resultView.aaa,
    });
  });
